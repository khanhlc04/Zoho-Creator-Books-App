...
document.querySelector('.search-bar').addEventListener('keypress', async function(event) {
    if (event.key === 'Enter' || event.key === 'Go') {
        ...
        window.open(`https://anlnhoubookapp.zohocreatorportal.com/#Page:Test?key=${this.value}`);
    }
});
...
async function createDocuments(){
    await ZOHO.CREATOR.init().then(async function() {
        ...
        await ZOHO.CREATOR.API.getAllRecords(configGetDocument).then(async function(response){
            ...
            if (recordDocuments.length > 0) {
                for(let i = 0; i < recordDocuments.length; i++) {
                    ...
                    const favDocumentBlock = document.createElement('div');
                    favDocumentBlock .className = 'inner-wrap';
                    ...
                    documentBlock.innerHTML = `
                             <a href="https://anlnhoubookapp.zohocreatorportal.com/#Page:Detail_Document?docID=${documentRecord.Document_ID}"  target="_blank">
                                    ...
                                    <a href="${fileUrl}" class="inner-download">
                    ...
                    const section_3_homepage = document.querySelector('.section-3-homepage');
                    section_3_homepage.querySelector('.container').appendChild(documentBlock);
                }
            }
        });
        ...
        await ZOHO.CREATOR.API.getAllRecords(configGetFavoriteDocument).then(async function(response){
            const recordFavorite = response.data;
            if (recordFavorite.length > 0) {
                for(let i = 0; i < recordFavorite.length; i++) {
                    let favoriteRecord = recordFavorite[i];
                    // URL của tệp tải xuống
                    ...
                    const configQueryDocument = {
                        appName: "database-books-app",
                        reportName: "All_Documents",
                        criteria: "(Document_ID == \""+ favoriteRecord.Document_ID + "\")"
                    }
            
                    await ZOHO.CREATOR.API.getAllRecords(configQueryDocument).then(async function(response){
                        const recordDocuments = response.data;
                        if (recordDocuments.length > 0) {
                            for(let i = 0; i < recordDocuments.length; i++) {
                                let documentRecord = recordDocuments[i];
                                ...
                                `;
                                const section_4_homepage = document.querySelector('.section-4-homepage');
                                section_4_homepage.querySelector('.container').appendChild(favDocumentBlock);
                            }
                        }
                    });
                }
