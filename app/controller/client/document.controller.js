...
async function createDocuments(){
    await ZOHO.CREATOR.init().then(async function() {
        ...
        if(queryParams.key) keyword = queryParams.key;
        ...
        if(keyword){
            configGetDocument = { 
                ...
                criteria : `(Deleted == \"false\" && Status == \"Active\" && Intellectual_Property_Agreement == \"true\" && Title.contains("${keyword}"))`,
                page: pageNumber,
                pageSize: pageSize
            }
        } else {
...
async function pagination(){
    await ZOHO.CREATOR.init().then(async function() {
        ...
        const configCountDocument = { 
            appName : "database-books-app",
            reportName : "All_Documents",
            criteria : `(Deleted == \"false\" && Status == \"Active\" && Intellectual_Property_Agreement == \"true\" && Title.contains("${keyword}"))`,
        }

        await ZOHO.CREATOR.API.getRecordCount(configCountDocument).then(function(response) {
            cnt = response.result.records_count;
        });
        ...
        for(let i = 1; i <= Math.ceil(cnt/pageSize); i++){
            pagination.innerHTML += `<span class="page-item" data-page="${i}">${i}</span>`;
        }
        
        pagination.innerHTML += `<span class="page-item" data-page="${Math.ceil(cnt/pageSize)}">Trang Cuối</span>`
